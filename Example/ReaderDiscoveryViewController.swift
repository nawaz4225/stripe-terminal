...
class ReaderDiscoveryViewController: TableViewController, DiscoveryDelegate, CancelableViewController, BluetoothReaderDelegate {
...
//        let connectionConfig = ConnectionConfiguration(failIfInUse: false)
        ...
        if reader.deviceType == .wisePad3 || reader.deviceType == .chipper2X {
            Terminal.shared.connectBluetoothReader(
                reader,
                delegate: self,
                connectionConfig: nil
            ) { connectedReader, error in
                    self.connectReader(reader, connectedReader: connectedReader, error: error)
            }
            } else {
    ...
    private func connectReader(_ reader: Reader, connectedReader: Reader?, error: Error? ) {
        if let connectedReader: Reader = connectedReader {
            self.cancelable = nil
