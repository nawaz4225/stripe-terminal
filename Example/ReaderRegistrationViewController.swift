...
class ReaderRegistrationViewController: TableViewController, DiscoveryDelegate, CancelableViewController, BluetoothReaderDelegate {
    ...
    func terminal(_ terminal: Terminal, didUpdateDiscoveredReaders readers: [Reader]) {
        ...
        if reader.deviceType == .wisePad3 || reader.deviceType == .chipper2X {
            Terminal.shared.connectInternetReader(
                reader,
                connectionConfig: nil) { connectedReader, error in
                self.setAllowedCancelMethods(.all)
                if let error = error {
